...
export async function getDealContext(text) {
  ...
  const withoutNumber = text.match(/(?:on|for|about|at)\s+([\w\s]+(?:cir|st|ave|blvd|dr|ln|rd|way|ct|park|glen|hill|lake|ridge|terrace)\b[^,\n?]*)/i);
  const m = withNumber || withoutNumber;
  if (!m) return null;
  const searchTerm = (m[1] || m[0]).trim().toLowerCase();
