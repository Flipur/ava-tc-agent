...
async function updateAssignmentTabs(token, accountId, envelopeId, assignor, fields) {
  const f = fields || {};
  const existingTabs = assignor.tabs?.textTabs || [];

  // Build update using the LIVE tabId from this envelope, matched by tabLabel
  const textTabs = [];
  for (const tab of existingTabs) {
    ...
    if (!value) continue;
    textTabs.push({
      tabId: tab.tabId,       // use live tabId from this envelope
      value: String(value),
      locked: "true",
    });
  }

  console.log(`Updating ${textTabs.length} tabs on recipient ${assignor.recipientId}`);

  const res = await fetch(
    `${DOCUSIGN_BASE_URL}/accounts/${accountId}/envelopes/${envelopeId}/recipients/${assignor.recipientId}/tabs`,
    {
      method: "PUT",
      headers: {
...
export async function createDocuSignEnvelope({ signerEmail, signerName, documentName, emailSubject, fields }) {
  ...
  // Step 2 — fetch live recipients with tabs, find Assignor, update tabs
  if (type === "assignment" && fields) {
    ...
    // Pick the Assignor with the most tabs (the real one, not the duplicate)
    ...
    if (assignor) {
      ...
      await updateAssignmentTabs(token, accountId, envelope.envelopeId, assignor, fields);
    } else {
      console.log("Assignor not found");
    }
  }
